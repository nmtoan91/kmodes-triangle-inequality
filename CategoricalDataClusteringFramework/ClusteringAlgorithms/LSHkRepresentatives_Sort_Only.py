...
from collections import defaultdict
import TDef
class LSHkRepresentatives_Sort_Only(ClusteringAlgorithm):
    ...
    def DoCluster(self,n_group=2):
        ...
        self.name = "LSHkRepresentatives_Sort_Only"
...
def Test_Simple():
    ...
    print("\n\n############## LSHkRepresentatives_Sort_Only ###################")
    lshkrepresentatives = LSHkRepresentatives_Sort_Only(DB['DB'],DB['labels_'] )
...
def TestDatasets(): 
    for dbname in MeasureManager.DATASET_LIST:
        ...
        MeasureManager.CURRENT_MEASURE = 'DILCA'
        print("\n\n############## LSHkRepresentatives_Sort_Only ###################")
        alo = LSHkRepresentatives_Sort_Only(DB['DB'],DB['labels_'],dbname=MeasureManager.CURRENT_DATASET ,k=TDef.k )
...
def TestMeasures():
    ...
    for measure in MeasureManager.MEASURE_LIST:
        ...
        for dataset in MeasureManager.DATASET_LIST:
            ...
            DB = tulti.LoadRealData(MeasureManager.CURRENT_DATASET)
            alo = LSHkRepresentatives_Sort_Only(DB['DB'],DB['labels_'],dbname=MeasureManager.CURRENT_DATASET )
            alo.SetupMeasure(MeasureManager.CURRENT_MEASURE)
            alo.SetupLSH(measure=MeasureManager.CURRENT_MEASURE)
            alo.DoCluster()
            alo.CalcScore()

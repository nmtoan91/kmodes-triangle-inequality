#TOANSTT Copy idead from MH-k-Kmodes (2)
import os
import os.path
import sys
...
class LSHkRepresentatives_Init(ClusteringAlgorithm):
    ...
    def DoCluster(self):
        ...
        for init_no in range(self.n_init):
            ...
            for key, value in self.lsh.hashTable.items():
                nearest_key=-1;
                nearest_dist = float('inf')
                for keymaster in masterkeys:
                    d_temp = self.lsh.hammingDistance(keymaster,key)
                    if d_temp < nearest_dist: nearest_dist= d_temp; nearest_key = keymaster;
...
def Test_Simple():
    DB = tulti.LoadSynthesisData(n=2048,d=20,k=16,sigma_rate=0.1); 

#Note by toanstt: LSH -> Original kRepresentatives
#Not good
...
class LSHkRepresentatives_Resolver(ClusteringAlgorithm):
    ...
    def DistanceRepresentativestoAPoints_LSH2(self,item_id, point,labels_matrix,representatives):
        ...
        for i in myset:
            ...
            if dist_min > dist:
                ...
                dist_index = i
        return dist_index, dist_min 
    def UpdateLabels_Original(self,representatives, X,representatives_sum, representatives_count,membship,labels_matrix):
        cost = 0
        move = 0
    ...
    def DoCluster(self,n_group=2,init_type ='random'):
        ...
        self.name = "kRepresentatives_Init3"
        ...
        print("LSHkRepresentatives_Resolver n_group=" + str(n_group) + " Average neighbors:" + str(self.k/n_group) )
        for init_no in range(self.n_init):
            start_time_init = timeit.default_timer()
            if TDef.verbose >=1: print ('LSHkRepresentatives_Resolver Init ' + str(init_no))
            ...
            for ki_ in range(ddd,ddd_end):
                ki = ki_%self.k
                if TDef.verbose >=3 and ki % (1000)==0 : print ('Preprocessing near neighbors: ', ki)
            ...
            #from DrawGraph import DrawGra
            #DrawGra(self.near_clusters)
            ...
            if TDef.verbose >=1: print ('LSHkRepresentatives_Resolver Init timelapse: ' + str(timeit.default_timer() -start_time_init))
            for i in range(self.n_iter):
                ...
                if last_cost <= cost or move==0:
                    ...
                    break 
                last_cost = cost
            #self.time_score = (timeit.default_timer() - start_time)/ self.n_init;self.labels = self.GetLabels(membship);self.CalcScore()
            #Resolver
            print("Beging resolver stage")
            for i in range(self.n_iter):
                start_time_iter = timeit.default_timer()
                self.iter = i
                cost ,move, count_empty = self.UpdateLabels_Original(representatives, X,representatives_sum, representatives_count,membship,labels_matrix)
                self.UpdateRepresentatives(representatives,representatives_sum,representatives_count ) ;
...
def Test_Simple():
    ...
    print("\n\n############## LSHkRepresentatives_Resolver ###################")
    lshkrepresentatives = LSHkRepresentatives_Resolver(DB['DB'],DB['labels_'] )
...
def Test(): 
    ...
    print("\n\n############## LSHkRepresentatives_Resolver ###################")
    lshkrepresentatives = LSHkRepresentatives_Resolver(DB['DB'],DB['labels_'] ,dbname=MeasureManager.CURRENT_DATASET ,k=TDef.k)
    lshkrepresentatives.SetupMeasure(MeasureManager.CURRENT_MEASURE)
    lshkrepresentatives.SetupLSH(measure=MeasureManager.CURRENT_MEASURE)
...
def TestDatasets(): 
    for dbname in MeasureManager.DATASET_LIST:
        ...
        MeasureManager.CURRENT_MEASURE = 'Overlap'
        print("\n\n############## LSHkRepresentatives_Resolver ###################")
        alo = LSHkRepresentatives_Resolver(DB['DB'],DB['labels_'],dbname=MeasureManager.CURRENT_DATASET )

...
class LSHkRepresentatives_Init2(ClusteringAlgorithm):
    ...
    def DoCluster(self,n_group=2):
        ...
        self.name = "kRepresentatives_Init2"
        ...
        print("LSHkRepresentatives_Init2 n_group=" + str(n_group) + " Average neighbors:" + str(self.k/n_group) )
        for init_no in range(self.n_init):
            start_time_init = timeit.default_timer()
            if TDef.verbose >=1: print ('LSHkRepresentatives_Init2 Init ' + str(init_no))
            ...
            allkeys = self.lsh.hashTable.keys()
            dist_from_master_to_master = np.zeros((self.k,self.k))
            dist_from_master_to_other = np.zeros((self.k,len(allkeys)))
            for i in range(self.k):
                for j in range(i+1,self.k):
                    dist_from_master_to_master[i][j]= dist_from_master_to_master[j][i] = \
                    self.lsh.hammingDistance(masterkeys[i],masterkeys[j])
            dist_from_master_to_other = [[ self.lsh.hammingDistance(keymaster,key) for key in allkeys] for keymaster in masterkeys  ]
            ...
            for ki_ in range(ddd,ddd_end):
                ki = ki_%self.k
                if TDef.verbose >=3 and ki % (100)==0 : print ('Preprocessing 1: ', ki)
            ...
            for key_id, key in enumerate(self.lsh.hashTable.keys()):
                if TDef.verbose >=3 and key_id % (100)==0 : print ('Preprocessing 2: ', key_id)
                nearest_key=-1;
            ...
            if TDef.verbose >=1: print ('LSHkRepresentatives_Init2 Init timelapse: ' + str(timeit.default_timer() -start_time_init))
...
def Test_Simple():
    ...
    print("\n\n############## LSHkRepresentatives_Init2 ###################")
    lshkrepresentatives = LSHkRepresentatives_Init2(DB['DB'],DB['labels_'] )
...
def Test(): 
    ...
    print("\n\n############## LSHkRepresentatives_Init2 ###################")
    lshkrepresentatives = LSHkRepresentatives_Init2(DB['DB'],DB['labels_'] ,dbname=MeasureManager.CURRENT_DATASET ,k=TDef.k)
    lshkrepresentatives.SetupMeasure(MeasureManager.CURRENT_MEASURE)
    lshkrepresentatives.SetupLSH(measure=MeasureManager.CURRENT_MEASURE)
    lshkrepresentatives.DoCluster(n_group=TDef.n_group)
    lshkrepresentatives.CalcScore()
...
def TestDatasets(): 
    for dbname in MeasureManager.DATASET_LIST:
        ...
        MeasureManager.CURRENT_MEASURE = 'Overlap'
        print("\n\n############## LSHkRepresentatives_Init2 ###################")
        alo = LSHkRepresentatives_Init2(DB['DB'],DB['labels_'],dbname=MeasureManager.CURRENT_DATASET )

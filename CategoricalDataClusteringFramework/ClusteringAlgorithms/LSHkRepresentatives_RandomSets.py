#Note by toanstt: Swap the near cluster list after each iterator
...
class LSHkRepresentatives_RandomSets(ClusteringAlgorithm):
    ...
    def CheckEmptyClusters_Init(self,representatives, X,representatives_sum, representatives_count,membship,labels_matrix):
        ...
        #for ki in range(self.k):
        #    if representatives_sum[ki] ==0 :
        #        if TDef.verbose >=3 :  print("Found a empty cluster (Loop): ", ki)
        #        big_cluster_id = np.argmax([sum(mem_) for mem_ in membship])
        #        choices = [i for i in range(self.n) if membship[big_cluster_id][i] == 1 ]
        #        rindx = int(self.farest_point[big_cluster_id][0])
        #        self.MovePoint(rindx, big_cluster_id,ki, representatives_count, representatives_sum,membship,self.X[rindx],labels_matrix  )
    ...
    def DistanceRepresentativestoAPoints_LSH2(self,item_id, point,labels_matrix,representatives):
        ...
        for i in myset:
            ...
            if dist_min > dist:
                ...
                dist_index = i
        return dist_index, dist_min 
    def DistanceRepresentativestoAPoints_RandomSet(self,item_id, point,labels_matrix,representatives):
        dist_min = self.Distance(representatives[labels_matrix[item_id]], point)
        dist_index =labels_matrix[item_id]
        myset = self.randomsets[(item_id+ self.iter) %self.groupcount]
        for i in myset:
            dist = self.Distance(representatives[i], point)
            if dist_min > dist:
                dist_min = dist 
    ...
    def UpdateLabels(self,representatives, X,representatives_sum, representatives_count,membship,labels_matrix):
        ...
        for ipoint, curpoint in enumerate(X):
            ...
            representative_id,tmp = self.DistanceRepresentativestoAPoints_RandomSet(ipoint, curpoint,labels_matrix,representatives)
    ...
    def DoCluster(self,n_group=2,init_type ='random'):
        ...
        self.name = "LSHkRepresentatives_RandomSets"
        ...
        print("LSHkRepresentatives_RandomSets n_group=" + str(n_group) + " Average neighbors:" + str(self.k/n_group) )
        for init_no in range(self.n_init):
            start_time_init = timeit.default_timer()
            if TDef.verbose >=1: print ('LSHkRepresentatives_RandomSets Init ' + str(init_no))
            ...
            # Calc near clusters
            masterkeys=[]
            self.groupcount = n_group
            n_group = int(self.k/n_group)
            #Random sets
            self.randomsets = [[-1  for j in range(n_group)]for i in range(self.groupcount)]
            for i in range(self.groupcount):
                for j in range(n_group): 
                    self.randomsets[i][j] = (i*n_group+j)%self.k
            #Fixed near clusters
            ...
            if TDef.verbose >=1: print ('LSHkRepresentatives_RandomSets Init timelapse: ' + str(timeit.default_timer() -start_time_init))
...
def Test_Simple():
    ...
    print("\n\n############## LSHkRepresentatives_RandomSets ###################")
    lshkrepresentatives = LSHkRepresentatives_RandomSets(DB['DB'],DB['labels_'] )
...
def Test(): 
    ...
    print("\n\n############## LSHkRepresentatives_RandomSets ###################")
    lshkrepresentatives = LSHkRepresentatives_RandomSets(DB['DB'],DB['labels_'] ,dbname=MeasureManager.CURRENT_DATASET ,k=TDef.k)
    lshkrepresentatives.SetupMeasure(MeasureManager.CURRENT_MEASURE)
    lshkrepresentatives.SetupLSH(measure=MeasureManager.CURRENT_MEASURE)
...
def TestDatasets(): 
    for dbname in MeasureManager.DATASET_LIST:
        ...
        MeasureManager.CURRENT_MEASURE = 'Overlap'
        print("\n\n############## LSHkRepresentatives_RandomSets ###################")
        alo = LSHkRepresentatives_RandomSets(DB['DB'],DB['labels_'],dbname=MeasureManager.CURRENT_DATASET )

...
class kModesTriangleInequality_IEEEAccess(ClusteringAlgorithm):
    ...
    def DoCluster(self,seed = 41,init_clusters=None):
        ...
        self.name = 'kModesTriangleInequality_IEEEAccess_Floyd'
        ...
        for iter in range(self.n_iter):
            dists =  sklearn.metrics.pairwise_distances(self.X, c, metric = overlapMetric)
            a = np.argmin(dists,1)
            y.fill(0)
            z.fill(0)
            for i in range(self.n):                
                y[a[i]] = y[a[i]] + self.X[i] #toanstt: Cannot work with categorical data
                z[a[i]] = z[a[i]] + 1

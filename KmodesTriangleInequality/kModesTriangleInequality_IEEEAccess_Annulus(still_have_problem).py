...
class kModesTriangleInequality_IEEEAccess(ClusteringAlgorithm):
    ...
    def DoCluster(self,seed = 41,init_clusters=None):
        ...
        self.name = 'kModesTriangleInequality_IEEEAccess_Annulus'
        ...
        for iter in range(self.n_iter):
            ...
            for i in range(self.n):
                z = max(l[i], s[a[i]])
                if u[i] <= z: continue
                u[i] = overlapMetric(c[a[i]], self.X[i])
                if u[i] <= z: continue
                l[i] = overlapMetric(self.X[i], c[b[i]])
                r = max(l[i], u[i])
                J = []
                #13 toanstt: What is O????
                for k in range(self.k):
                    if overlapMetric(self.X[i], )
